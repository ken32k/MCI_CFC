...
# @FilePath: /csvd-sfc/svd_lm_wmh_vol.sh
...
for sub_dir in ${sub_list[@]}; do
    ...
    # WMH DWI metrics
    dwi_dir=${PROJ_HOME}/csvd_mica_out/micapipe_v0.2.0/${sub_id}/dwi
    ...
    # NAWM
    wmparc_mgz=${PROJ_HOME}/csvd_mica_out/freesurfer/${sub_id}/mri/wmparc.mgz
    if [ ! -f "$wmparc_mgz" ]; then
        echo "[Error] ... ${sub_dir}: wmparc file not found, pass."
        continue
    fi
    dst_nifti=${PROJ_HOME}/csvd_mica_out/micapipe_v0.2.0/${sub_id}/anat/${sub_id}_cerebral_WM.nii.gz
    wmh_nifti=${PROJ_HOME}/csvd_mica_out/micapipe_v0.2.0/${sub_id}/anat/${sub_id}_space-5tt_wmh.nii.gz
    nawm_bin=${PROJ_HOME}/csvd_mica_out/micapipe_v0.2.0/${sub_id}/anat/${sub_id}_cerebral_WM_bin.nii.gz
    if [ ! -f "$nawm_bin" ]; then
    ...
    # # T1FLAIR
    # tfr_raw=${PROJ_HOME}/csvd_mica_bids/T1T2FLAIRMTR_ratio/${sub_id}/ses-${sub_id}/${sub_id}_ratio-T1FLAIR_calib-nonlin.nii.gz
    # tfr_anat=${PROJ_HOME}/csvd_mica_out/micapipe_v0.2.0/${sub_id}/anat/${sub_id}_ratio-T1FLAIR_calib-nonlin.nii.gz
    # tfr_mask=${PROJ_HOME}/csvd_mica_out/micapipe_v0.2.0/${sub_id}/anat/${sub_id}_ratio-T1FLAIR_mask.nii.gz
    # if [ -f $tfr_raw ]; then
