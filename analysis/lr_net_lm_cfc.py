...
def par_net_cfc_lrm(cm_list, sub_id, lm_lab):
    ...
    for yeo_net_name in ana_utils.YEO7_DICT.keys():
        ...
        for cm in cm_list:
            ...
            cm = cm_mat[cm_idx][ana_utils.YEO7_DICT[yeo_net_name], :]
            cm_mats.append(cm)
        # Get fc
        fc_mat = np.load(
            f"{ana_utils.PROJ_HOME}/data/fcs/{sub_id}_fc-wb_{ana_utils.ATLAS}.npy"
        )[ana_utils.YEO7_DICT[yeo_net_name], :]

        # fc_mat[fc_mat < 0.2 * np.max(fc_mat)] = 0
        # fc_mat[fc_mat < 0] = 0
        # Get rsq, pval and dominance
        try:
            rsq_val, p_val, dominance, model_r_sq, len_nan = (
                ana_utils.sub_mat_linear_reg(
                    # np.power(fc_mat,2),
                    fc_mat,
                    np.array(cm_mats),
                )
...
def get_cfc_lrm(cm_list, lm_lab):
    ...
    for idx, sub_id in enumerate(sub_cm_list):
        ...
        cfc_results = np.append(cfc_results, par_net_cfc_lrm(cm_list, sub_id, lm_lab))
...
for i, lm_lab in enumerate(ana_utils.LM_LAB):
    ...
    # neuro comabt
    # cfc_results_tbl = ana_utils.harm_combat_net(cfc_results_tbl)
